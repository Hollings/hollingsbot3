...
from .identity import generate_bot_identity
from .names import departure_message
from .summary import generate_conversation_summary, save_conversation_summary
...
class TempBotManager:
    ...
    def __init__(self, bot: commands.Bot, coordinator: Any, typing_tracker: Any):
        ...
        # Persistent debug log storage (message_id -> log data)
        self.debug_log_store = DebugLogStore(_DEBUG_LOGS_PATH, max_size=_DEBUG_LOGS_MAX_SIZE)
    ...
    def get_response_log(self, message_id: int) -> dict[str, Any] | None:
        ...
        return self.debug_log_store.get(message_id)
    ...
    async def _generate_response(self, conversation: list[dict[str, Any]]) -> str:
        ...
        """
        text, _llm_debug, _conv = await self._generate_response_with_job(conversation, GenerationJob())
        return text
    ...
    async def _cleanup_temp_bot(
        ...
        # Mark inactive FIRST, before any await that could be cancelled.
        # If a new message arrives mid-cleanup, _cancel_generation will try to
        # increment_temp_bot_replies as a refund - but that query has
        # `WHERE is_active = 1`, so the refund becomes a no-op once we're inactive.
        # Without this, the bot can be revived mid-cleanup and respond forever.
        ...
        if channel_id:
            ...
            summary = await generate_conversation_summary(channel_id, bot_name, spawn_prompt)
            if summary:
                await save_conversation_summary(webhook_id, summary, append=is_recall)
