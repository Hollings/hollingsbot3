...
class DebugCommands(commands.Cog):
    ...
    def _format_debug_files(self, log_data: dict[str, object]) -> tuple[discord.File, discord.File]:
        ...
        try:
            with open(MESSAGE_LOG_FILE) as f:
                for line in f:
                    ...
                    try:
                        entry = json.loads(line)
                        if entry.get("channel_id") == channel_id and entry.get("outbox_ts", 0) < before_ts:
                            previous = entry
                    except json.JSONDecodeError:
                        continue
        except Exception as e:
    ...
    async def _handle_wendy_debug(self, message: discord.Message, target_msg_id: int) -> bool:
        ...
        prev_msg = self._find_previous_wendy_message(msg_info["channel_id"], msg_info["outbox_ts"])
