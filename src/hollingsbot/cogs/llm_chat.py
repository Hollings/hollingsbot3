...
class LLMChatNewCog(commands.Cog):
    ...
    async def _generate_and_send_response(
        ...
        assistant_turn = await self._deliver_response(channel, text, tool_messages)
    ...
    async def _send_response_to_channel(
        self, channel: discord.abc.MessageableChannel, text: str, svg_files: list[discord.File]
    ...
    async def _deliver_response(
        self, channel: discord.abc.MessageableChannel, raw_text: str, tool_messages: list[str]
    ) -> ConversationTurn:
        ...
        # Send to channel
        sent_messages = await self._send_response_to_channel(channel, text, svg_files)
