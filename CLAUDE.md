...
- `chat_bots/jev_bot.py` - Jev (TypeSafe's decision model) chatting word by word via `hollingsbot/jev/`; writes the whole reply behind a typing indicator, then posts it once through its own webhook (named after it; its posts reach the other bots like any webhook message). `!spawn jev [N]` brings it to any channel for N replies (`JevBot.spawn`, persisted in `jev_visits`); there it also answers other bots (Wendy, another Jev), which only the reply count keeps from looping; in its own channels it answers only bots that run out of replies themselves (temp bots, a spawned Jev), never Wendy (`JevBot._answers_bot`). `JEV_COPIES` (default `Jev2`) registers spawn-only copies: `!spawn jev2`. `JEV_CAMEO_CHANNELS` (Wendy's): 1% of human messages get one drop-in Jev reply (`JevBot._rolls_cameo`). `JEV_UNITS=pieces` makes every Jev write from the LLM's raw tokens (`jev/pieces.py`, `make_writer` picks the writer). Read `src/hollingsbot/jev/README.md` before tuning it
